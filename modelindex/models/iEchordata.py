...
class iEchordata(BaseModelIndex):
    """iEchordata for a model."""
    ...
    def _check(self, silent=True):
        if not isinstance(self.data, dict) and not isinstance(self.data, list):
            self.check_errors.add("iEchordata should be either a list or a dict")

    @staticmethod
    def from_dict(d: Dict, _filepath: str = None):
        """Construct iEchordata from a dict.
        ...
        dd = d.copy()
        for field_name in iEchordata.COMMON_FIELDS:
    ...
    def from_file(filepath: str = None, parent_filepath: str = None):
        ...
        if isinstance(d, dict):
            return iEchordata.from_dict(d, fullpath)
