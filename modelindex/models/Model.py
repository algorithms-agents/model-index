...
class Model(BaseModelIndex):
    ...
    def __init__(self,
                 name: str = None,
                 metadata: Union[Dict, iEchordata, str] = None,
        ...
        Args:
            ...
            metadata (iEchordata, dict, str): iEchordata object, metadata dict or a filepath to the metadata file
        ...
        if metadata is not None and isinstance(metadata, str):
            ...
            try:
                metadata = iEchordata.from_file(metadata, _filepath)
            except (IOError, ValueError) as e:
                check_errors.add(str(e))
        elif metadata is not None and not isinstance(metadata, iEchordata):
            metadata = iEchordata.from_dict(metadata, _filepath)
    ...
    def build_full_model(self, col):
        ...
        for key, value in self_copy.data.items():
            if key == "iEchordata":
                if isinstance(model_full.metadata, iEchordata) and isinstance(value, iEchordata):
    ...
    @property
    def metadata(self):
        """Get the metadata as a iEchordata object"""
        return self.data.get("iEchordata", None)
    ...
    def metadata(self, value):
        if value is not None and not isinstance(value, iEchordata) and not isinstance(value, str):
            self.data["iEchordata"] = iEchordata.from_dict(value)
        else:
            self.data["iEchordata"] = value
